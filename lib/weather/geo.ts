...
export async function resolveCityFromGeo(
  lat: number,
  lon: number,
  hintCityName?: string
): Promise<City> {
  const match = await reverseGeocodeCoords(lat, lon, hintCityName);
  const placeName = match?.name ?? hintCityName ?? `Место ${lat.toFixed(2)}, ${lon.toFixed(2)}`;
