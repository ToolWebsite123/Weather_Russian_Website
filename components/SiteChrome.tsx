      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-3 py-2 sm:px-6">
              ...
              <div className="absolute left-0 top-full mt-1.5 w-64 sm:w-80 rounded-xl border border-sky-100 bg-white py-1.5 shadow-xl z-50 max-h-72 overflow-y-auto">
                {searchResults.map((r) => (
                  <Link
                    key={`${r.id}-${r.slug}`}
                    href={buildCityUrl(r)}
                    className="block px-3 py-2 text-left hover:bg-sky-50 transition-colors border-b border-slate-100 last:border-0"
                    onClick={() => {
                      setIsSearchFocused(false);
                      setSearchQuery("");
                    }}
                  >
                    <div className="text-xs sm:text-sm font-semibold text-sky-950">{r.name}</div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <button
            type="button"
            onClick={toggleUnit}
            className="flex items-center justify-center h-8 sm:h-9 rounded-xl bg-sky-100/90 border border-sky-200/80 px-2.5 text-xs font-bold text-sky-900 hover:bg-sky-200 transition-colors cursor-pointer"
