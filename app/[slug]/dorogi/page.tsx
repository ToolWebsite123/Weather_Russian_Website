...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Состояние дорог и погода для водителей ${locative} | WeatherHub`;
  const description = `Оценка состояния дорожного покрытия ${locative}: температура асфальта, риск гололедицы и видимость.`;
