...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода ${locative} на месяц — долгосрочный прогноз | WeatherHub`;
  const description = `Прогноз погоды ${locative} на месяц: средние температуры, климатические нормы и тренд осадков.`;
...
export default function MesyatsWeatherPage({ params }: Props) {
  return (
    <CityWeatherView
      slug={params.slug}
      active="mesyats"
