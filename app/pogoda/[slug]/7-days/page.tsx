...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода ${locative} на неделю (7 дней) | WeatherHub`;
  const description = `Прогноз погоды ${locative} на неделю (7 дней). Узнайте колебания температуры, осадки и порывы ветра.`;
  const url = `${config.siteUrl}/pogoda/${city.slug}/7-days`;
  ...
  return {
    ...
    alternates: { canonical: `${config.siteUrl}/pogoda/${city.slug}/7-dney` },
    openGraph: {
      title,
      description,
      url,
...
export default function SevenDaysAliasPage({ params }: Props) {
  return (
    <CityWeatherView
      slug={params.slug}
      active="7"
