...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода ${locative} на 14 дней (2 недели) | WeatherHub`;
  const description = `Долгосрочный прогноз погоды ${locative} на 14 дней (2 недели).`;
  const url = `${config.siteUrl}/pogoda/${city.slug}/14-days`;
