...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода ${locative} на 3 дня — краткосрочный прогноз | WeatherHub`;
  const description = `Точный прогноз погоды ${locative} на 3 дня. График температуры, вероятность дождей и ветра.`;
  const url = `${config.siteUrl}/pogoda/${city.slug}/3-days`;
  ...
  return {
    ...
    alternates: { canonical: `${config.siteUrl}/pogoda/${city.slug}/3-dnya` },
    openGraph: {
      title,
      description,
      url,
...
export default function ThreeDaysAliasPage({ params }: Props) {
  return (
    <CityWeatherView
      slug={params.slug}
      active="3"
