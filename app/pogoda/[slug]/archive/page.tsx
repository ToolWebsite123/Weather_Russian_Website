...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Архив погоды ${locative} — климатическая история | WeatherHub`;
  const description = `Архив погоды ${locative}. Просмотр исторических данных о температуре, осадках и давлении за прошлые годы.`;
  const url = `${config.siteUrl}/pogoda/${city.slug}/archive`;
  ...
  return {
    ...
    alternates: { canonical: `${config.siteUrl}/pogoda/${city.slug}/archiv` },
    openGraph: {
      title,
      description,
      url,
...
export default function ArchiveAliasPage({ params }: Props) {
