...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода ${locative} на месяц — прогноз температуры на 30 дней | WeatherHub`;
  const description = `Прогноз погоды ${locative} на месяц. Климатический обзор и тренды на 30 дней.`;
  const url = `${config.siteUrl}/pogoda/${city.slug}/month`;
  ...
  return {
    ...
    alternates: { canonical: `${config.siteUrl}/pogoda/${city.slug}/mesyats` },
    openGraph: {
      title,
      description,
      url,
...
export default function MonthAliasPage({ params }: Props) {
  return (
    <CityWeatherView
      slug={params.slug}
      active="mesyats"
