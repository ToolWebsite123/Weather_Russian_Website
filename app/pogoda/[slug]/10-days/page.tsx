...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода ${locative} на 10 дней — подробный декадный прогноз | WeatherHub`;
  const description = `Прогноз погоды ${locative} на 10 дней: тренды температуры, осадки и давление на 10 суток.`;
  const url = `${config.siteUrl}/pogoda/${city.slug}/10-days`;
  ...
  return {
    ...
    alternates: { canonical: `${config.siteUrl}/pogoda/${city.slug}/10-dney` },
    openGraph: {
      title,
      description,
      url,
...
export default function TenDaysAliasPage({ params }: Props) {
  return (
    <CityWeatherView
      slug={params.slug}
      active="10"
