...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода на дорогах ${locative} — дорожные условия | WeatherHub`;
  const description = `Прогноз дорожных условий ${locative}: температура покрытия, риск гололедицы, видимость на трассах и порывы ветра.`;
  const url = `${config.siteUrl}/pogoda/${city.slug}/road`;
  ...
  return {
    ...
    alternates: { canonical: `${config.siteUrl}/pogoda/${city.slug}/dorogi` },
    openGraph: {
      title,
      description,
      url,
...
export default function RoadAliasPage({ params }: Props) {
