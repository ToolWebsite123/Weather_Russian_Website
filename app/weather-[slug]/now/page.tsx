...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода ${locative} сейчас — метеонаблюдения онлайн | WeatherHub`;
  const description = `Текущая погода ${locative} прямо сейчас: температура воздуха, влажность, давление и ветер.`;
  const url = `${config.siteUrl}${buildCityUrl(city, "now")}`;
...
export default function NowWeatherPage({ params }: Props) {
  return <CityWeatherView slug={params.slug} active="now" />;
}
