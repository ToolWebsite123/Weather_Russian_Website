...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = `Прогноз погоды ${locative} на неделю: график температур, осадки, влажность и атмосферное давление.`;
  const url = `${config.siteUrl}${buildCityUrl(city, "7-days")}`;
...
export default function SevenDneyWeatherPage({ params }: Props) {
  return (
    <CityWeatherView
      slug={params.slug}
      active="7-dney"
