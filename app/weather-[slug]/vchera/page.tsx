...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода ${locative} вчера — архив фактических данных | WeatherHub`;
  const description = `Фактическая погода ${locative} вчера: почасовая температура, влажность, давление и скорость ветра.`;
  const url = `${config.siteUrl}${buildCityUrl(city, "yesterday")}`;
...
export default function VcheraWeatherPage({ params }: Props) {
  return (
    <CityWeatherView
      slug={params.slug}
      active="vchera"
