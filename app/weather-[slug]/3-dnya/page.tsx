...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода ${locative} на 3 дня — точный метеопрогноз | WeatherHub`;
  const description = `Прогноз погоды ${locative} на 3 дня: подробная почасовая температура, вероятность осадков и скорость ветра.`;
  const url = `${config.siteUrl}${buildCityUrl(city, "3-days")}`;
...
export default function ThreeDnyaWeatherPage({ params }: Props) {
  return (
    <CityWeatherView
      slug={params.slug}
      active="3-dnya"
