...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода ${locative} на 2 недели — прогноз на 14 дней | WeatherHub`;
  const description = `Подробный прогноз погоды ${locative} на 2 недели (14 дней): температура, динамика осадков и атмосферное давление.`;
  const url = `${config.siteUrl}${buildCityUrl(city, "14-days")}`;
...
export default function TwoWeeksWeatherPage({ params }: Props) {
  return (
    <CityWeatherView
      slug={params.slug}
      active="14"
