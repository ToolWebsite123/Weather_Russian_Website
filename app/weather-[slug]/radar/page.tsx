...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Радар осадков онлайн ${locative} — интерактивная карта | WeatherHub`;
  const description = `Интерактивный радар и карта осадков ${locative} онлайн: движение грозовых туч и снегопадов.`;
  const url = `${config.siteUrl}${buildCityUrl(city, "radar")}`;
...
export default function RadarWeatherPage({ params }: Props) {
