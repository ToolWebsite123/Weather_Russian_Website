...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = `Прогноз погоды ${locative} на предстоящие выходные: субботу и воскресенье. Температура и осадки.`;
  const url = `${config.siteUrl}${buildCityUrl(city, "weekend")}`;
...
export default function VykhodnyeWeatherPage({ params }: Props) {
  return (
    <CityWeatherView
      slug={params.slug}
      active="vykhodnye"
