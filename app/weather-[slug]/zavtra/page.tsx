...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода ${locative} на завтра — точный прогноз | WeatherHub`;
  const description = `Прогноз погоды ${locative} на завтра: подробная почасовая температура, вероятность осадков и скорость ветра.`;
...
export default function ZavtraWeatherPage({ params }: Props) {
  return (
    <CityWeatherView
      slug={params.slug}
      active="zavtra"
