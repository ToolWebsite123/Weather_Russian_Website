...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Погода ${locative} на 14 дней — прогноз на 2 недели | WeatherHub`;
  const description = `Подробный прогноз погоды ${locative} на 14 дней (2 недели): температурные нормы, геомагнитные возмущения и УФ.`;
...
export default function FourteenDaysAliasPage({ params }: Props) {
  return (
    <CityWeatherView
      slug={params.slug}
      active="14"
